...
def align_vector_to_another(a=np.array([0, 0, 1]), b=np.array([1, 0, 0])):
    ...
    if np.allclose(a, b):  # much better
        return None, None
    axis_ = np.cross(a, b)  # cross product
    axis_ = axis_ / np.linalg.norm(axis_)  # normalise
    angle = np.arccos(np.dot(a, b))  # class arc cos angle between two vectors
...
class LineMesh(object):
    ...
    def create_line_mesh(self):
        ...
        line_segments_unit, line_lengths = normalized(line_segments)  # todo why normalise. I don't understand much here
        ...
        for i in range(line_segments_unit.shape[0]):
            ...
            translation = first_points[i, :] + line_segment * line_length * 0.5  # compensate for center
